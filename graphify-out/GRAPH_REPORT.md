# Graph Report - task-buddy-frontend  (2026-05-19)

## Corpus Check
- 158 files · ~163,652 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1737 nodes · 2719 edges · 142 communities (137 shown, 5 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 2 edges (avg confidence: 0.9)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3e660843`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Authentication & Authorization|Authentication & Authorization]]
- [[_COMMUNITY_Core UI Components|Core UI Components]]
- [[_COMMUNITY_User Navigation & Theme|User Navigation & Theme]]
- [[_COMMUNITY_Audit & Activity Logging|Audit & Activity Logging]]
- [[_COMMUNITY_Task Management Forms|Task Management Forms]]
- [[_COMMUNITY_State Management & Actions|State Management & Actions]]
- [[_COMMUNITY_Theme Utilities|Theme Utilities]]
- [[_COMMUNITY_Testing & Validation|Testing & Validation]]
- [[_COMMUNITY_API Documentation & Integration|API Documentation & Integration]]
- [[_COMMUNITY_Main Application Shell|Main Application Shell]]
- [[_COMMUNITY_Data Models & Entities|Data Models & Entities]]
- [[_COMMUNITY_Linting Configuration|Linting Configuration]]
- [[_COMMUNITY_Playwright Testing Config|Playwright Testing Config]]
- [[_COMMUNITY_Tailwind CSS Styling|Tailwind CSS Styling]]
- [[_COMMUNITY_Build & Vite Configuration|Build & Vite Configuration]]
- [[_COMMUNITY_Project Documentation|Project Documentation]]
- [[_COMMUNITY_Community 16|Community 16]]
- [[_COMMUNITY_Community 17|Community 17]]
- [[_COMMUNITY_Community 18|Community 18]]
- [[_COMMUNITY_Community 19|Community 19]]
- [[_COMMUNITY_Community 20|Community 20]]
- [[_COMMUNITY_Community 21|Community 21]]
- [[_COMMUNITY_Community 22|Community 22]]
- [[_COMMUNITY_Community 23|Community 23]]
- [[_COMMUNITY_Community 24|Community 24]]
- [[_COMMUNITY_Community 25|Community 25]]
- [[_COMMUNITY_Community 26|Community 26]]
- [[_COMMUNITY_Community 27|Community 27]]
- [[_COMMUNITY_Community 28|Community 28]]
- [[_COMMUNITY_Community 29|Community 29]]
- [[_COMMUNITY_Community 30|Community 30]]
- [[_COMMUNITY_Community 31|Community 31]]
- [[_COMMUNITY_Community 32|Community 32]]
- [[_COMMUNITY_Community 33|Community 33]]
- [[_COMMUNITY_Community 34|Community 34]]
- [[_COMMUNITY_Community 35|Community 35]]
- [[_COMMUNITY_Community 36|Community 36]]
- [[_COMMUNITY_Community 37|Community 37]]
- [[_COMMUNITY_Community 38|Community 38]]
- [[_COMMUNITY_Community 39|Community 39]]
- [[_COMMUNITY_Community 40|Community 40]]
- [[_COMMUNITY_Community 41|Community 41]]
- [[_COMMUNITY_Community 42|Community 42]]
- [[_COMMUNITY_Community 43|Community 43]]
- [[_COMMUNITY_Community 44|Community 44]]
- [[_COMMUNITY_Community 45|Community 45]]
- [[_COMMUNITY_Community 46|Community 46]]
- [[_COMMUNITY_Community 47|Community 47]]
- [[_COMMUNITY_Community 48|Community 48]]
- [[_COMMUNITY_Community 49|Community 49]]
- [[_COMMUNITY_Community 50|Community 50]]
- [[_COMMUNITY_Community 51|Community 51]]
- [[_COMMUNITY_Community 52|Community 52]]
- [[_COMMUNITY_Community 53|Community 53]]
- [[_COMMUNITY_Community 54|Community 54]]
- [[_COMMUNITY_Community 55|Community 55]]
- [[_COMMUNITY_Community 56|Community 56]]
- [[_COMMUNITY_Community 57|Community 57]]
- [[_COMMUNITY_Community 58|Community 58]]
- [[_COMMUNITY_Community 59|Community 59]]
- [[_COMMUNITY_Community 60|Community 60]]
- [[_COMMUNITY_Community 61|Community 61]]
- [[_COMMUNITY_Community 62|Community 62]]
- [[_COMMUNITY_Community 63|Community 63]]
- [[_COMMUNITY_Community 64|Community 64]]
- [[_COMMUNITY_Community 65|Community 65]]
- [[_COMMUNITY_Community 66|Community 66]]
- [[_COMMUNITY_Community 67|Community 67]]
- [[_COMMUNITY_Community 68|Community 68]]
- [[_COMMUNITY_Community 69|Community 69]]
- [[_COMMUNITY_Community 70|Community 70]]
- [[_COMMUNITY_Community 71|Community 71]]
- [[_COMMUNITY_Community 72|Community 72]]
- [[_COMMUNITY_Community 73|Community 73]]
- [[_COMMUNITY_Community 74|Community 74]]
- [[_COMMUNITY_Community 75|Community 75]]
- [[_COMMUNITY_Community 76|Community 76]]
- [[_COMMUNITY_Community 77|Community 77]]
- [[_COMMUNITY_Community 78|Community 78]]
- [[_COMMUNITY_Community 79|Community 79]]
- [[_COMMUNITY_Community 80|Community 80]]
- [[_COMMUNITY_Community 81|Community 81]]
- [[_COMMUNITY_Community 82|Community 82]]
- [[_COMMUNITY_Community 83|Community 83]]
- [[_COMMUNITY_Community 84|Community 84]]
- [[_COMMUNITY_Community 85|Community 85]]
- [[_COMMUNITY_Community 86|Community 86]]
- [[_COMMUNITY_Community 87|Community 87]]
- [[_COMMUNITY_Community 88|Community 88]]
- [[_COMMUNITY_Community 89|Community 89]]
- [[_COMMUNITY_Community 90|Community 90]]
- [[_COMMUNITY_Community 91|Community 91]]
- [[_COMMUNITY_Community 92|Community 92]]
- [[_COMMUNITY_Community 93|Community 93]]
- [[_COMMUNITY_Community 94|Community 94]]
- [[_COMMUNITY_Community 95|Community 95]]
- [[_COMMUNITY_Community 96|Community 96]]
- [[_COMMUNITY_Community 97|Community 97]]
- [[_COMMUNITY_Community 98|Community 98]]
- [[_COMMUNITY_Community 99|Community 99]]
- [[_COMMUNITY_Community 100|Community 100]]
- [[_COMMUNITY_Community 101|Community 101]]
- [[_COMMUNITY_Community 102|Community 102]]
- [[_COMMUNITY_Community 103|Community 103]]
- [[_COMMUNITY_Community 104|Community 104]]
- [[_COMMUNITY_Community 105|Community 105]]
- [[_COMMUNITY_Community 106|Community 106]]
- [[_COMMUNITY_Community 107|Community 107]]
- [[_COMMUNITY_Community 108|Community 108]]
- [[_COMMUNITY_Community 109|Community 109]]
- [[_COMMUNITY_Community 110|Community 110]]
- [[_COMMUNITY_Community 111|Community 111]]
- [[_COMMUNITY_Community 112|Community 112]]
- [[_COMMUNITY_Community 113|Community 113]]
- [[_COMMUNITY_Community 114|Community 114]]
- [[_COMMUNITY_Community 115|Community 115]]
- [[_COMMUNITY_Community 116|Community 116]]
- [[_COMMUNITY_Community 117|Community 117]]
- [[_COMMUNITY_Community 118|Community 118]]
- [[_COMMUNITY_Community 119|Community 119]]
- [[_COMMUNITY_Community 120|Community 120]]
- [[_COMMUNITY_Community 121|Community 121]]
- [[_COMMUNITY_Community 122|Community 122]]
- [[_COMMUNITY_Community 123|Community 123]]
- [[_COMMUNITY_Community 124|Community 124]]
- [[_COMMUNITY_Community 125|Community 125]]
- [[_COMMUNITY_Community 126|Community 126]]
- [[_COMMUNITY_Community 127|Community 127]]
- [[_COMMUNITY_Community 128|Community 128]]
- [[_COMMUNITY_Community 130|Community 130]]
- [[_COMMUNITY_Community 131|Community 131]]
- [[_COMMUNITY_Community 132|Community 132]]
- [[_COMMUNITY_Community 141|Community 141]]

## God Nodes (most connected - your core abstractions)
1. `useToast()` - 56 edges
2. `useAuth()` - 54 edges
3. `cn()` - 49 edges
4. `useTaskDrawerState()` - 28 edges
5. `toast()` - 26 edges
6. `Task` - 18 edges
7. `Tag` - 16 edges
8. `Task-Buddy Frontend` - 15 edges
9. `TasksPage()` - 14 edges
10. `useFilters()` - 13 edges

## Surprising Connections (you probably didn't know these)
- `TaskBuddy App` --references--> `App Icon`  [INFERRED]
  src/App.tsx → public/task-buddy-icon.svg
- `API Hooks` --implements--> `Backend API Docs`  [INFERRED]
  src/hooks/useApi.ts → API.md
- `handleSubmit()` --calls--> `toast()`  [EXTRACTED]
  src/components/create-project-modal.tsx → src/hooks/use-toast.ts
- `handleNewTaskClick()` --calls--> `toast()`  [EXTRACTED]
  src/components/layout/mobile-nav.tsx → src/hooks/use-toast.ts
- `handleSubmit()` --calls--> `toast()`  [EXTRACTED]
  src/pages/ForgotPasswordPage.tsx → src/hooks/use-toast.ts

## Communities (142 total, 5 thin omitted)

### Community 0 - "Authentication & Authorization"
Cohesion: 0.05
Nodes (78): { activeStatus, setActiveStatus }, [activeTab, setActiveTab], [confirmData, setConfirmData], Dashboard(), DashboardProps, filteredTasks, handleDelete, handleDeleteSubtask (+70 more)

### Community 1 - "Core UI Components"
Cohesion: 0.04
Nodes (45): attachTag, createSubtask, createTag, createTask, { data: allTags = [] }, { data: fetchedTask }, { data: projects = [] }, [deleteSnapshot, setDeleteSnapshot] (+37 more)

### Community 2 - "User Navigation & Theme"
Cohesion: 0.05
Nodes (44): markAllRead, { notifications, unreadCount }, handleClick(), iconMap, markRead, navigate, NotificationItemProps, useMarkAllRead() (+36 more)

### Community 3 - "Audit & Activity Logging"
Cohesion: 0.05
Nodes (35): { 
    activeSidebarFilter, 
    setActiveSidebarFilter,
    activeTagId,
    setActiveTagId 
  }, { 
    activeSidebarFilter, 
    setActiveSidebarFilter,
    activeTagId,
    setActiveTagId,
    setSelectedPriorities,
  }, { 
    activeSidebarFilter, 
    setActiveSidebarFilter,
    activeTagId,
    setActiveTagId,
    setSelectedPriorities,
    setSelectedProjects,
    setSelectedTags
  }, categories, { data: groups = [] }, { data: projects = [] }, { data: projectTasks = [], isLoading: isLoadingTasks }, { data: tags = [] } (+27 more)

### Community 4 - "Task Management Forms"
Cohesion: 0.06
Nodes (30): actionsTrigger, deleteBtn, deleteButtons, DeleteConfirmationModalProps, deleteModalButton, editButtons, mockSetActiveSidebarFilter, mockSetActiveTagId (+22 more)

### Community 5 - "State Management & Actions"
Cohesion: 0.11
Nodes (18): ColorIconPickerProps, cn(), result, SidebarBrandingProps, GripHandle, GripHandleProps, ProjectsSectionProps, SmartLink (+10 more)

### Community 6 - "Theme Utilities"
Cohesion: 0.07
Nodes (27): SettingsContext, SettingsContextType, SettingsProvider(), { result }, TimeFormat, useSettings(), displayTime, [h24, m] (+19 more)

### Community 7 - "Testing & Validation"
Cohesion: 0.06
Nodes (31): AuditEntry, AuditTrailProps, controller, [currentLimit, setCurrentLimit], [error, setError], EXCLUDED_ACTIONS, fetchAuditLog, fieldChanges (+23 more)

### Community 8 - "API Documentation & Integration"
Cohesion: 0.2
Nodes (29): Sidebar(), useToast(), useProjectManagement(), useCreateProject(), useProjects(), useSubtaskManagement(), useTagManagement(), useCreateTag() (+21 more)

### Community 9 - "Main Application Shell"
Cohesion: 0.06
Nodes (30): API Endpoints, Base URL Configuration, code:typescript (interface Task {), code:typescript (const { deleteTask, loading, error } = useDeleteTask();), code:typescript (const { tasks, loading, error } = useTasks();), code:block12 (Content-Type: application/json), code:env (VITE_API_BASE_URL=http://127.0.0.1:8000), code:typescript (const { createTask, loading } = useCreateTask();) (+22 more)

### Community 10 - "Data Models & Entities"
Cohesion: 0.07
Nodes (27): { 
    activeSidebarFilter, 
    activeStatus, 
    setActiveStatus, 
    activeTagId,
  }, [confirmData, setConfirmData], { data: projects = [] }, { data: tags = [] }, { data: tasks = [], isLoading: loadingTasks, refetch: refreshTasks }, handleConfirmAction(), { handleEditTask }, handleToggleComplete() (+19 more)

### Community 11 - "Linting Configuration"
Cohesion: 0.09
Nodes (18): PRESET_COLORS, [color, setColor], COLORS, createProject, CreateProjectModalProps, handleSubmit(), [icon, setIcon], [lastOpen, setLastOpen] (+10 more)

### Community 12 - "Playwright Testing Config"
Cohesion: 0.11
Nodes (16): UseSubtaskManagementReturn, UseTaskDirtyStateProps, UseTaskDrawerActionsProps, UseTaskDrawerActionsReturn, Group, isLogoutRequest, mutatingMethods, statsApi (+8 more)

### Community 13 - "Tailwind CSS Styling"
Cohesion: 0.12
Nodes (10): [confirmationInput, setConfirmationInput], ConfirmationModalProps, [dontShowAgain, setDontShowAgain], [shake, setShake], DeleteConfirmationModalProps, AlertDialog(), AlertDialogCancel(), extraProps (+2 more)

### Community 14 - "Build & Vite Configuration"
Cohesion: 0.09
Nodes (18): BeforeInstallPromptEvent, [deferredPrompt, setDeferredPrompt], handler, isAndroidApp, [isIOS], [isIOS, setIsIOS], [isStandalone], [isStandalone, setIsStandalone] (+10 more)

### Community 15 - "Project Documentation"
Cohesion: 0.1
Nodes (18): [category, setCategory], { data: groups = [] }, { data: projects = [] }, [description, setDescription], [dueDate, setDueDate], [groupId, setGroupId], { groups }, [lastOpen, setLastOpen] (+10 more)

### Community 16 - "Community 16"
Cohesion: 0.09
Nodes (21): Additional Forbidden Patterns, Anti-Patterns (Do NOT Use), Buttons, Cards, code:css (@import url('https://fonts.googleapis.com/css2?family=Fira+C), code:css (/* Primary Button */), code:css (.card {), code:css (.input {) (+13 more)

### Community 17 - "Community 17"
Cohesion: 0.1
Nodes (19): act, bold(), createdName, date, fieldChanges, fieldsMatch, getAuditIcon(), handleTaskUpdate() (+11 more)

### Community 18 - "Community 18"
Cohesion: 0.11
Nodes (20): [confirmPassword, setConfirmPassword], confirmPasswordError, [email, setEmail], emailError, [password, setPassword], passwordError, passwordStrength, { register, loading } (+12 more)

### Community 19 - "Community 19"
Cohesion: 0.1
Nodes (19): A premium productivity application with a dual-identity design:, Brand Identity, Cards, clean & minimal in light mode, deep navy & warm amber in dark mode., Color Usage Rules, Component Patterns, Dual-Mode Identity, Empty States (+11 more)

### Community 20 - "Community 20"
Cohesion: 0.12
Nodes (6): useTasks(), tasksApi, mockTask, mockTasks, { result }, task

### Community 21 - "Community 21"
Cohesion: 0.16
Nodes (13): ThemeProvider(), AuthContext, AuthContextType, AuthProvider(), AuthUser, BackendErrorDetail, extractAccessToken(), formatFirstBackendError() (+5 more)

### Community 22 - "Community 22"
Cohesion: 0.14
Nodes (15): {
    data: task,
    isLoading,
    error,
  }, deleteMutation, { id }, [isDeleting, setIsDeleting], navigate, priorityColors, queryClient, { toast } (+7 more)

### Community 23 - "Community 23"
Cohesion: 0.12
Nodes (10): categoryConfig, [pendingAction, setPendingAction], priorityConfig, [showAllSubtasks, setShowAllSubtasks], [showConfirm, setShowConfirm], TaskCard(), TaskCardProps, { timeFormat } (+2 more)

### Community 24 - "Community 24"
Cohesion: 0.12
Nodes (13): DirtySections, [hours, minutes], MetaSidebar(), MetaSidebarProps, newDate, [popoverOpen, setPopoverOpen], PRIORITY_STYLES, quickDates (+5 more)

### Community 25 - "Community 25"
Cohesion: 0.11
Nodes (17): [confirmPassword, setConfirmPassword], confirmPasswordError, [email, setEmail], emailError, met, PASSWORD_RULES, [password, setPassword], passwordError (+9 more)

### Community 26 - "Community 26"
Cohesion: 0.12
Nodes (16): code:html (<!-- In <head> -->), code:bash (git commit -m "chore: finalize mobile nav integration"), code:css (/* Add to end of src/index.css */), code:bash (git add index.html src/index.css), code:tsx (import { motion } from "framer-motion"), code:bash (git add src/components/layout/mobile-nav.tsx), code:tsx (import { Sheet, SheetContent, SheetHeader, SheetTitle } from), code:bash (git add src/components/layout/mobile-drawer.tsx) (+8 more)

### Community 27 - "Community 27"
Cohesion: 0.17
Nodes (14): sanitizePassword(), validatePassword(), [email, setEmail], emailError, handleSubmit(), { login, loading }, { login, loading, error }, navigate (+6 more)

### Community 28 - "Community 28"
Cohesion: 0.16
Nodes (8): ResolvedTheme, Theme, THEME_VALUES, ThemeProviderContext, ThemeProviderProps, ThemeProviderState, useTheme(), ThemeToggle()

### Community 29 - "Community 29"
Cohesion: 0.14
Nodes (13): createButton, filtered, method, mockTags, mockTasks, newTask, payload, subtaskCheckbox (+5 more)

### Community 30 - "Community 30"
Cohesion: 0.14
Nodes (13): 1) Architectural Style, 2) System Flow, 3) Layer/Module Responsibilities, 4) Reused Patterns, 5) Graphify Insights (Core Abstractions), 5) Known Architectural Risks, 6) Evidence, 6) Known Architectural Risks (+5 more)

### Community 31 - "Community 31"
Cohesion: 0.15
Nodes (9): [color, setColor], COLORS, createGroup, CreateGroupModalProps, [name, setName], { toast }, useCreateGroup(), useGroups() (+1 more)

### Community 32 - "Community 32"
Cohesion: 0.18
Nodes (8): SidebarItemActions(), SidebarItemActionsProps, onDelete, onEdit, parentClick, trigger, user, DropdownMenu()

### Community 33 - "Community 33"
Cohesion: 0.17
Nodes (12): PRESET_ICONS, [color, setColor], createTag, CreateTagModal(), CreateTagModalProps, handleSubmit(), [icon, setIcon], [lastOpen, setLastOpen] (+4 more)

### Community 34 - "Community 34"
Cohesion: 0.27
Nodes (9): useDeleteProject(), useReorderProjects(), useSidebarActions(), useDeleteTag(), useReorderTags(), stores, UserPreferences, useUserPreferences() (+1 more)

### Community 35 - "Community 35"
Cohesion: 0.15
Nodes (12): Browser Support, code:env (# API Configuration), Color Palette, Contributing, Design System, Environment Variables, License, Overview (+4 more)

### Community 36 - "Community 36"
Cohesion: 0.15
Nodes (12): Audit Trail Context & Time Formatting Implementation Plan, code:typescript (// src/lib/audit-trail-helpers.tsx), code:bash (git add src/lib/audit-trail-helpers.tsx), code:typescript (// src/components/audit-trail.tsx), code:bash (git add src/components/audit-trail.tsx), code:typescript (// src/components/task-card.tsx), code:bash (git add src/components/task-card.tsx), code:bash (npm run lint) (+4 more)

### Community 37 - "Community 37"
Cohesion: 0.15
Nodes (12): code:typescript (import { renderHook, act } from "@testing-library/react"), code:typescript (import React, { createContext, useContext, useState, useEffe), code:bash (git add src/contexts/SettingsContext.tsx src/contexts/Settin), code:typescript (// src/App.tsx), code:bash (git add src/App.tsx), code:typescript (// src/pages/ProfilePage.tsx), code:typescript (export function ProfilePage() {), code:bash (git add src/pages/ProfilePage.tsx) (+4 more)

### Community 38 - "Community 38"
Cohesion: 0.17
Nodes (12): handleConfirmAction(), handleSubmit(), handleConfirmDelete(), handleAddSubtask(), handleAttachTag(), handleCreate(), handleCreateAndAttachTag(), handleDelete() (+4 more)

### Community 39 - "Community 39"
Cohesion: 0.18
Nodes (7): [confirmationInput, setConfirmationInput], DeleteProjectModalProps, [deleteTasks, setDeleteTasks], [lastOpen, setLastOpen], [shake, setShake], LogoutDialogProps, animations

### Community 40 - "Community 40"
Cohesion: 0.2
Nodes (11): Action, ActionType, actionTypes, addToRemoveQueue(), dispatch(), listeners, memoryState, reducer() (+3 more)

### Community 41 - "Community 41"
Cohesion: 0.17
Nodes (11): handleA, handleB, handles, mockProjects, mockTags, projectA, projectB, projectsSection (+3 more)

### Community 42 - "Community 42"
Cohesion: 0.18
Nodes (11): API Integration, Architecture, code:block1 (src/), code:typescript (// Fetch all tasks), `dashboard.tsx`, Key Components, `new-task-modal.tsx`, Project Structure (+3 more)

### Community 43 - "Community 43"
Cohesion: 0.18
Nodes (10): 1) Naming Rules, 2) Formatting and Linting, 3) Import and Module Conventions, 4) Error and Logging Conventions, 5) Testing Conventions, 6) Evidence, code:bash (npm run lint), Coding Conventions (+2 more)

### Community 44 - "Community 44"
Cohesion: 0.18
Nodes (10): 1) Test Stack and Commands, 2) Test Layout, 3) Test Scope Matrix, 4) Mocking and Isolation Strategy, 5) Coverage and Quality Signals, 6) Evidence, code:bash (npm test              # Run Unit tests), Core Sections (Required) (+2 more)

### Community 45 - "Community 45"
Cohesion: 0.18
Nodes (10): 1. Purpose, 2. Coverage Targets, 3. Coverage Theater Prevention, 4. Fitness-to-Purpose Scenarios, 5. AI Session Quality Discipline, 6. The Human Gate, Quality Constitution: Task Buddy, Scenario 1: The Mid-Write Crash (Resilience) (+2 more)

### Community 46 - "Community 46"
Cohesion: 0.24
Nodes (6): met, PASSWORD_RULES, PasswordStrengthMeterProps, { score, label }, getPasswordStrength(), ResetPasswordPage()

### Community 47 - "Community 47"
Cohesion: 0.2
Nodes (10): Build, code:bash (# Install dependencies), code:env (VITE_API_BASE_URL=http://127.0.0.1:8000), code:bash (# Start dev server), code:bash (# Build for production), Configuration, Development, Getting Started (+2 more)

### Community 48 - "Community 48"
Cohesion: 0.2
Nodes (9): Color Overrides, Component Overrides, Layout Overrides, Page-Specific Components, Page-Specific Rules, Recommendations, Spacing Overrides, Tasks Page Overrides (+1 more)

### Community 49 - "Community 49"
Cohesion: 0.2
Nodes (9): 1) Top Risks (Prioritized), 2) Technical Debt, 3) Security Concerns, 4) Performance and Scaling Concerns, 5) Fragile/High-Churn Areas, 6) `[ASK USER]` Questions, 7) Evidence, Codebase Concerns (+1 more)

### Community 50 - "Community 50"
Cohesion: 0.2
Nodes (9): 1) Integration Inventory, 2) Data Stores, 3) Secrets and Credentials Handling, 4) Reliability and Failure Behavior, 5) Observability for Integrations, 6) Evidence, Core Sections (Required), Extended Sections (Optional) (+1 more)

### Community 51 - "Community 51"
Cohesion: 0.2
Nodes (9): 1) Runtime Summary, 2) Production Frameworks and Dependencies, 3) Development Toolchain, 4) Key Commands, 5) Environment and Config, 6) Evidence, code:bash (npm install), Core Sections (Required) (+1 more)

### Community 52 - "Community 52"
Cohesion: 0.2
Nodes (9): code:block1 (Test timeout of 30000ms exceeded.), code:block2 (Error: locator.click: Test timeout of 30000ms exceeded.), code:yaml (- generic [ref=e2]:), code:ts (1  | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 53 - "Community 53"
Cohesion: 0.2
Nodes (9): code:block1 (Test timeout of 30000ms exceeded.), code:block2 (Error: locator.click: Test timeout of 30000ms exceeded.), code:yaml (- generic [ref=e2]:), code:ts (1  | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 54 - "Community 54"
Cohesion: 0.2
Nodes (9): code:block1 (Test timeout of 30000ms exceeded.), code:block2 (Error: locator.click: Test timeout of 30000ms exceeded.), code:yaml (- generic [ref=e2]:), code:ts (1  | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 55 - "Community 55"
Cohesion: 0.2
Nodes (9): code:block1 (Test timeout of 30000ms exceeded.), code:block2 (Error: locator.fill: Test timeout of 30000ms exceeded.), code:yaml (- generic [ref=e2]:), code:ts (1   | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 56 - "Community 56"
Cohesion: 0.2
Nodes (9): code:block1 (Test timeout of 30000ms exceeded.), code:block2 (Error: locator.fill: Test timeout of 30000ms exceeded.), code:yaml (- generic [ref=e2]:), code:ts (1   | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 57 - "Community 57"
Cohesion: 0.24
Nodes (9): code:block1 (Test timeout of 30000ms exceeded.), code:block2 (Error: locator.fill: Test timeout of 30000ms exceeded.), code:yaml (- generic [ref=e2]:), code:ts (1   | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 58 - "Community 58"
Cohesion: 0.2
Nodes (9): code:block1 (Test timeout of 30000ms exceeded.), code:block2 (Error: locator.click: Test timeout of 30000ms exceeded.), code:yaml (- generic [ref=e2]:), code:ts (1  | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 59 - "Community 59"
Cohesion: 0.2
Nodes (9): code:block1 (Test timeout of 30000ms exceeded.), code:block2 (Error: locator.click: Test timeout of 30000ms exceeded.), code:yaml (- generic [ref=e2]:), code:ts (1  | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 60 - "Community 60"
Cohesion: 0.2
Nodes (9): code:block1 (Test timeout of 30000ms exceeded.), code:block2 (Error: locator.click: Test timeout of 30000ms exceeded.), code:yaml (- generic [ref=e2]:), code:ts (1  | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 61 - "Community 61"
Cohesion: 0.2
Nodes (9): code:block1 (Test timeout of 30000ms exceeded.), code:block2 (Error: locator.fill: Test timeout of 30000ms exceeded.), code:yaml (- generic [ref=e2]:), code:ts (1   | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 62 - "Community 62"
Cohesion: 0.2
Nodes (9): code:block1 (TypeError: Cannot read properties of null (reading 'complete), code:block2 (Error: page.waitForTimeout: Target page, context or browser ), code:yaml (- generic [ref=e2]:), code:ts (42  |     await page.route("**/api/v1/tasks/**", async (rout), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 63 - "Community 63"
Cohesion: 0.24
Nodes (9): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (29  |       globalThis.localStorage.setItem('auth_user', JSO), code:ts (42  |     await page.route("**/api/v1/tasks/**", async (rout), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 64 - "Community 64"
Cohesion: 0.2
Nodes (9): code:block1 (Test timeout of 30000ms exceeded.), code:block2 (Error: locator.fill: Test timeout of 30000ms exceeded.), code:yaml (- generic [ref=e2]:), code:ts (1   | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info (+1 more)

### Community 65 - "Community 65"
Cohesion: 0.28
Nodes (6): SortMode, TasksHeaderProps, Badge(), BadgeProps, badgeVariants, InputProps

### Community 66 - "Community 66"
Cohesion: 0.22
Nodes (8): 1. Karpathy Behavioral Guidelines, 2. Type Safety & Standards, 3. Accessibility (WCAG), 4. React 19 Compatibility, 5. Code Quality & Patterns, 6. Project-Specific Context, code:tsx (// Correct), Frontend Standards & Guidelines (Task Buddy)

### Community 67 - "Community 67"
Cohesion: 0.22
Nodes (9): Add a new UI component, Add API integration, Adding New Features, code:bash (# Create in src/components/ui/), code:typescript (// Add new hook to src/hooks/useApi.ts), code:css (:root {), Common Tasks, Customize colors (+1 more)

### Community 68 - "Community 68"
Cohesion: 0.22
Nodes (8): 1) Top-Level Map, 2) Entry Points, 3) Module Boundaries, 4) Naming and Organization Rules, 5) Evidence, Codebase Structure, Core Sections (Required), Extended Sections (Optional)

### Community 69 - "Community 69"
Cohesion: 0.22
Nodes (8): Automated Tests, code:tsx (import { useSettings } from "@/contexts/SettingsContext"), Manual Verification, Task 1: Refactor TimePicker with Format Support and Suggestions, Task 2: Integrate Time Format in NewTaskModal, Task 3: Integrate Time Format in MetaSidebar, Time System Refinement Implementation Plan, Verification Plan

### Community 70 - "Community 70"
Cohesion: 0.22
Nodes (8): Application Integration, Architecture, Data Flow, Global Settings Management Design, ProfilePage Preferences Section, SettingsContext, Success Criteria, Testing Strategy

### Community 71 - "Community 71"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (29  |       globalThis.localStorage.setItem('auth_user', JSO), Error details, Instructions, Page snapshot, Test info, Test source

### Community 72 - "Community 72"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (29  |       globalThis.localStorage.setItem('auth_user', JSO), Error details, Instructions, Page snapshot, Test info, Test source

### Community 73 - "Community 73"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (29  |       globalThis.localStorage.setItem('auth_user', JSO), Error details, Instructions, Page snapshot, Test info, Test source

### Community 74 - "Community 74"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (29  |       globalThis.localStorage.setItem('auth_user', JSO), Error details, Instructions, Page snapshot, Test info, Test source

### Community 75 - "Community 75"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (29  |       globalThis.localStorage.setItem('auth_user', JSO), Error details, Instructions, Page snapshot, Test info, Test source

### Community 76 - "Community 76"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toHaveCount(expected) failed), code:yaml (- generic [ref=e2]:), code:ts (5   |     { id: 1, name: "Project A", color: "#FF0000", icon), Error details, Instructions, Page snapshot, Test info, Test source

### Community 77 - "Community 77"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic:), code:ts (73  |           const subtaskId = Number.parseInt(url.pathna), Error details, Instructions, Page snapshot, Test info, Test source

### Community 78 - "Community 78"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (1  | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info, Test source

### Community 79 - "Community 79"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toHaveCount(expected) failed), code:yaml (- generic [ref=e2]:), code:ts (33  |           status: 200,), Error details, Instructions, Page snapshot, Test info, Test source

### Community 80 - "Community 80"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (1   | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info, Test source

### Community 81 - "Community 81"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeEnabled() failed), code:yaml (- generic [ref=e2]:), code:ts (4   |   test.beforeEach(async ({ page }) => {), Error details, Instructions, Page snapshot, Test info, Test source

### Community 82 - "Community 82"
Cohesion: 0.22
Nodes (7): code:block1 (Error: expect(locator).toBeDisabled() failed), code:yaml (- generic [ref=e2]:), Error details, Instructions, Page snapshot, Test info, Test source

### Community 83 - "Community 83"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (1  | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info, Test source

### Community 84 - "Community 84"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toContainText(expected) failed), code:yaml (- generic [ref=e2]:), code:ts (48  |           }),), Error details, Instructions, Page snapshot, Test info, Test source

### Community 85 - "Community 85"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (1  | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info, Test source

### Community 86 - "Community 86"
Cohesion: 0.22
Nodes (7): code:block1 (Error: expect(locator).toBeDisabled() failed), code:yaml (- generic [ref=e2]:), Error details, Instructions, Page snapshot, Test info, Test source

### Community 87 - "Community 87"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeEnabled() failed), code:yaml (- generic [ref=e2]:), code:ts (4   |   test.beforeEach(async ({ page }) => {), Error details, Instructions, Page snapshot, Test info, Test source

### Community 88 - "Community 88"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (1   | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info, Test source

### Community 89 - "Community 89"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toContainText(expected) failed), code:yaml (- generic [ref=e2]:), code:ts (48  |           }),), Error details, Instructions, Page snapshot, Test info, Test source

### Community 90 - "Community 90"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (1  | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info, Test source

### Community 91 - "Community 91"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (1  | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info, Test source

### Community 92 - "Community 92"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (1  | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info, Test source

### Community 93 - "Community 93"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toHaveCount(expected) failed), code:yaml (- generic [ref=e2]:), code:ts (5   |     { id: 1, name: "Project A", color: "#FF0000", icon), Error details, Instructions, Page snapshot, Test info, Test source

### Community 94 - "Community 94"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toHaveCount(expected) failed), code:yaml (- generic [ref=e2]:), code:ts (33  |           status: 200,), Error details, Instructions, Page snapshot, Test info, Test source

### Community 95 - "Community 95"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toHaveCount(expected) failed), code:yaml (- generic [ref=e2]:), code:ts (5   |     { id: 1, name: "Project A", color: "#FF0000", icon), Error details, Instructions, Page snapshot, Test info, Test source

### Community 96 - "Community 96"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeEnabled() failed), code:yaml (- generic [ref=e2]:), code:ts (4   |   test.beforeEach(async ({ page }) => {), Error details, Instructions, Page snapshot, Test info, Test source

### Community 97 - "Community 97"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toContainText(expected) failed), code:yaml (- generic [ref=e2]:), code:ts (48  |           }),), Error details, Instructions, Page snapshot, Test info, Test source

### Community 98 - "Community 98"
Cohesion: 0.22
Nodes (7): code:block1 (Error: expect(locator).toBeDisabled() failed), code:yaml (- generic [ref=e2]:), Error details, Instructions, Page snapshot, Test info, Test source

### Community 99 - "Community 99"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e2]:), code:ts (1   | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info, Test source

### Community 100 - "Community 100"
Cohesion: 0.22
Nodes (8): code:block1 (Error: locator.fill: Error: strict mode violation: getByLabe), code:yaml (- generic [ref=e5]:), code:ts (1   | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info, Test source

### Community 101 - "Community 101"
Cohesion: 0.22
Nodes (8): code:block1 (Error: locator.fill: Error: strict mode violation: getByLabe), code:yaml (- generic [ref=e5]:), code:ts (47  |             user: { id: "user-1", username: "demo.user), Error details, Instructions, Page snapshot, Test info, Test source

### Community 102 - "Community 102"
Cohesion: 0.22
Nodes (8): code:block1 (Error: locator.fill: Error: strict mode violation: getByLabe), code:yaml (- generic [ref=e5]:), code:ts (1   | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info, Test source

### Community 103 - "Community 103"
Cohesion: 0.22
Nodes (8): code:block1 (Error: expect(locator).toBeVisible() failed), code:yaml (- generic [ref=e5]:), code:ts (1   | import { test, expect } from "@playwright/test"), Error details, Instructions, Page snapshot, Test info, Test source

### Community 104 - "Community 104"
Cohesion: 0.22
Nodes (8): code:block1 (Error: locator.fill: Error: strict mode violation: getByLabe), code:yaml (- generic [ref=e5]:), code:ts (28  |   })), Error details, Instructions, Page snapshot, Test info, Test source

### Community 105 - "Community 105"
Cohesion: 0.25
Nodes (6): preferences, sensors, subtaskIds, SubtaskSection(), SubtaskSectionProps, { user }

### Community 106 - "Community 106"
Cohesion: 0.25
Nodes (6): api, [email, setEmail], handleSubmit(), [isLoading, setIsLoading], [isSubmitted, setIsSubmitted], { toast }

### Community 107 - "Community 107"
Cohesion: 0.25
Nodes (7): 1. Objective, 2.1 MobileNav (`components/layout/mobile-nav.tsx`), 2.2 MobileDrawer (`components/layout/mobile-drawer.tsx`), 2. Components, 3. Technical Integration, 4. Success Criteria, Design Spec: Mobile Navigation & PWA Polish

### Community 108 - "Community 108"
Cohesion: 0.38
Nodes (7): handleSubmit(), navigate, normalizeText(), sanitizeEmail(), sanitizeUsername(), handleSubmit(), navigate

### Community 109 - "Community 109"
Cohesion: 0.29
Nodes (6): code:typescript (export function useCreateTag() {), Sidebar & Tag Creation Implementation Plan, Task 1: API and Hook Infrastructure, Task 2: CreateTagModal Component, Task 3: Sidebar Refactor (Collapsibility & Dynamic Icons), Task 4: Centering & UI Polish

### Community 110 - "Community 110"
Cohesion: 0.47
Nodes (4): Toast, toastVariants, ToastViewport, { toasts }

### Community 111 - "Community 111"
Cohesion: 0.33
Nodes (5): badge, bell, mockNotifications, toggleButton, url

### Community 112 - "Community 112"
Cohesion: 0.33
Nodes (4): Core Workflows, Project: Task Buddy, Quality Playbook (READ FIRST), Tech Stack

### Community 113 - "Community 113"
Cohesion: 0.33
Nodes (5): code:tsx (import { Button } from "@/components/ui/button"), code:bash (git add src/components/task-drawer/MetaSidebar.tsx), Fix Missing Button Import in MetaSidebar Implementation Plan, Task 1: Add Button Import to MetaSidebar.tsx, Task 2: Verification

### Community 114 - "Community 114"
Cohesion: 0.33
Nodes (5): Execution UX, Field Reference Table, Protocol: Integration Testing (Task Buddy), Safety Constraints, Test Matrix

### Community 115 - "Community 115"
Cohesion: 0.47
Nodes (6): clearHubFilters(), handleLogout(), handleProjectClick(), handleSidebarFilterClick(), handleTagClick(), navigate

### Community 116 - "Community 116"
Cohesion: 0.4
Nodes (5): handleCreateProject(), handleDateSelect(), handleDescriptionBlur(), handleTitleBlur(), handleUpdate()

### Community 117 - "Community 117"
Cohesion: 0.4
Nodes (4): useAuditTrail(), UseAuditTrailOptions, AuditEntry, groupByDate()

### Community 118 - "Community 118"
Cohesion: 0.4
Nodes (5): Accessibility, Modern React Patterns, Performance, State Management, Type Safety

### Community 119 - "Community 119"
Cohesion: 0.4
Nodes (4): code:tsx (import {), code:bash (git add src/components/sidebar.tsx), Fix @dnd-kit/core Type Import in Sidebar Implementation Plan, Task 1: Fix Sidebar Imports

### Community 120 - "Community 120"
Cohesion: 0.4
Nodes (4): Focus Areas, Guardrails, Protocol: AI-Powered Code Review (Task Buddy), Regression Protocol

### Community 121 - "Community 121"
Cohesion: 0.4
Nodes (4): Protocol: Council of Three Spec Audit, Scrutiny Areas, The Council, Triage Rules

### Community 122 - "Community 122"
Cohesion: 0.5
Nodes (3): payload, strengthBar, strengthLabel

### Community 123 - "Community 123"
Cohesion: 0.5
Nodes (3): profileLink, sidebar, toggleButton

### Community 124 - "Community 124"
Cohesion: 0.5
Nodes (4): Dashboard Insights, Features, Task Management, User Experience

### Community 125 - "Community 125"
Cohesion: 0.5
Nodes (4): Audit Trail, Backend API Docs, API Hooks, Dashboard Page

### Community 127 - "Community 127"
Cohesion: 0.67
Nodes (3): TaskBuddy App, Auth Context, App Icon

### Community 128 - "Community 128"
Cohesion: 0.67
Nodes (3): Subtask Model, Tag Model, Task Model

## Knowledge Gaps
- **989 isolated node(s):** `mockTasks`, `{ result }`, `mockTask`, `task`, `LoginPage` (+984 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **5 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `useToast()` connect `API Documentation & Integration` to `Authentication & Authorization`, `Core UI Components`, `User Navigation & Theme`, `Audit & Activity Logging`, `Task Management Forms`, `Data Models & Entities`, `Linting Configuration`, `Playwright Testing Config`, `Project Documentation`, `Community 18`, `Community 20`, `Community 22`, `Community 25`, `Community 27`, `Community 31`, `Community 33`, `Community 34`, `Community 40`, `Community 46`, `Community 106`, `Community 110`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `cn()` connect `State Management & Actions` to `Authentication & Authorization`, `Core UI Components`, `User Navigation & Theme`, `Audit & Activity Logging`, `Task Management Forms`, `Theme Utilities`, `Testing & Validation`, `Data Models & Entities`, `Tailwind CSS Styling`, `Build & Vite Configuration`, `Project Documentation`, `Community 22`, `Community 23`, `Community 24`, `Community 32`, `Community 39`, `Community 65`, `Community 105`, `Community 110`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `useAuth()` connect `Authentication & Authorization` to `User Navigation & Theme`, `Community 34`, `Audit & Activity Logging`, `Testing & Validation`, `API Documentation & Integration`, `Community 105`, `Data Models & Entities`, `Linting Configuration`, `Community 18`, `Community 20`, `Community 117`, `Community 21`, `Community 25`, `Community 27`, `Community 31`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **What connects `mockTasks`, `{ result }`, `mockTask` to the rest of the system?**
  _989 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Authentication & Authorization` be split into smaller, more focused modules?**
  _Cohesion score 0.05 - nodes in this community are weakly interconnected._
- **Should `Core UI Components` be split into smaller, more focused modules?**
  _Cohesion score 0.04 - nodes in this community are weakly interconnected._
- **Should `User Navigation & Theme` be split into smaller, more focused modules?**
  _Cohesion score 0.05 - nodes in this community are weakly interconnected._