...
## Corpus Check
- 52 files · ~36,481 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 447 nodes · 671 edges · 32 communities (31 shown, 1 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 2 edges (avg confidence: 0.9)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ca565c38`
...
Cohesion: 0.05
Nodes (65): met, PASSWORD_RULES, PasswordStrengthMeterProps, { score, label }, RegisterForm(), AuthContext, AuthContextType, AuthUser (+57 more)

### Community 1 - "Core UI Components"
Cohesion: 0.06
Nodes (37): AuditEntry, AuditTrailProps, controller, [currentLimit, setCurrentLimit], [error, setError], filteredLogs, [loading, setLoading], [logs, setLogs] (+29 more)

### Community 2 - "User Navigation & Theme"
Cohesion: 0.13
Nodes (33): Dashboard(), DashboardProps, LogoutDialogProps, NewTaskModal(), Sidebar(), SidebarProps, SystemOverview(), TopNav() (+25 more)
...
## Knowledge Gaps
- **234 isolated node(s):** `{ token }`, `AuditEntry`, `AuditTrailProps`, `[logs, setLogs]`, `[currentLimit, setCurrentLimit]` (+229 more)
...
- **Why does `useAuth()` connect `User Navigation & Theme` to `Authentication & Authorization`, `Core UI Components`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `Task-Buddy Frontend` connect `Audit & Activity Logging` to `Testing & Validation`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
- **Why does `cn()` connect `Core UI Components` to `User Navigation & Theme`, `State Management & Actions`?**
