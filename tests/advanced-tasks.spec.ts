  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.route("**/api/v1/tasks/**", async (route) => {
      ...
      if (method === "GET") {
        ...
        if (tagId) {
          const filtered = mockTasks.filter((t) =>
            (t.tags as Record<string, unknown>[])?.some(
              (tg: Record<string, unknown>) => tg.id === Number.parseInt(tagId)
            )
          )
      ...
      if (method === "PUT" && url.pathname.includes("/subtask/")) {
        ...
        const payload = route.request().postDataJSON()
        console.log(
          `MOCK: Updating subtask ${subtaskId} to completed=${payload.completed}`
        )

        mockTasks.forEach((t) => {
          ;(t.subtasks as Record<string, unknown>[])?.forEach(
    ...
    await page.route("**/api/v1/stats/overview", async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          tag_distribution: mockTags.map((t) => ({
            tag_name: t.name,
            task_count: mockTasks.filter((tk) =>
              (tk.tags as Record<string, unknown>[])?.some(
                (tg: Record<string, unknown>) => tg.id === t.id
              )
            ).length,
          })),
        }),
    ...
    await expect(
      page.locator("text=Syncing your workspace...")
