...
test.describe("Main Layout & Navigation", () => {
  ...
  test("persistent layout stays mounted during navigation", async ({
    page,
  }) => {
    // 1. Mock Login
    await page.route("**/api/v1/users/token", async (route) => {
      await route.fulfill({
        ...
        headers: {
          "Set-Cookie":
            "access_token=test-token; HttpOnly; Path=/; SameSite=Lax",
        },
        contentType: "application/json",
        body: JSON.stringify({
    ...
    await test.step("Verify persistent elements are visible", async () => {
      await expect(
        page.getByRole("heading", { name: /welcome back, testuser/i })
      ).toBeVisible()
      await expect(
    ...
    await test.step("Navigate to profile and back, ensuring layout persistence", async () => {
      ...
      const sidebarPersistence = await page.getAttribute(
        "aside",
        "data-persistent"
      )
      ...
      // Still persistent
      const sidebarPersistenceBack = await page.getAttribute(
        "aside",
        "data-persistent"
      )
