...
function parseTagChange(details: string, name: string): React.ReactNode {
  ...
  return (
    <span>
      Added tag {tagName ? bold(tagName) : "a tag"} to{" "}
      {name ? bold(name) : "task"}
    </span>
  )
}
...
function parseFieldChanges(details: string, name: string): React.ReactNode {
  ...
  const fieldsMatch =
    /\| Changes:\s*(.+)$/i.exec(details) ||
    /\(fields:\s*([^)]+?)\)/i.exec(details)
  ...
  const parsedChanges = fieldChanges.map((change) => {
    ...
    if (match) {
      ...
      if (field === "due_date")
        return `Rescheduled from ${formatAuditDate(from)} to ${formatAuditDate(to)}`
      if (field === "project_id" || field === "project") return `Moved project`
      if (field === "completed")
        return to === "True" ? "Marked as done" : "Reopened"
...
function parseDateChange(details: string, name: string): React.ReactNode {
  const dateMatch =
    /due_date\s*from\s*['"]?([^'"]+)['"]?\s*to\s*['"]?([^'"]+)['"]?/i.exec(
      details
    )
...
function parseProjectChange(details: string, name: string): React.ReactNode {
  const projMatch =
    /project\s*from\s*['"]?([^'"]+)['"]?\s*to\s*['"]?([^'"]+)['"]?/i.exec(
      details
    )
  ...
  return (
    <span>
      Moved {name ? bold(name) : "task"} from {bold(projMatch[1])} to{" "}
      {bold(projMatch[2])}
    </span>
  )
}
...
function handleTaskUpdate(
  ...
  if (
    detLow.includes("status: completed") ||
    /\bcompleted:\s*['"]?true['"]?$/.test(detLow)
  )
    return <span>Marked {name ? bold(name) : "a task"} as done</span>
...
export function describeTaskAction(
  act: string,
  details: string
): React.ReactNode {
  ...
  if (act.includes("update"))
    return handleTaskUpdate(details?.toLowerCase() || "", details, name)
...
export function describeSubtaskAction(
  ...
  if (act.includes("update")) {
    if (
      details?.toLowerCase().includes("marked completed") ||
      details?.toLowerCase().includes("completed: false -> true")
    )
      return (
        <span>
          Finished subtask {subNode}
          {parentNode}
        </span>
      )
  ...
  return (
    <span>
      Activity on subtask {subNode}
      {parentNode}
    </span>
  )
}

export function describeProjectAction(
  act: string,
  details: string
): React.ReactNode {
  const name = getTargetName(details, "project")
  if (act.includes("create"))
    return <span>Created project {name ? bold(name) : "a project"}</span>
  if (act.includes("update")) {
    const fieldView = parseFieldChanges(details, name)
    if (fieldView) return fieldView
  ...
  if (act.includes("delete"))
    return <span>Deleted project {name ? bold(name) : "a project"}</span>
  return null
}
...
export function describeAction(
  ...
  if (target === "SUBTASK")
    return describeSubtaskAction(act, details) || details
  if (target === "PROJECT" || target === "GROUP")
    return describeProjectAction(act, details) || details

  const humanAction = action?.replaceAll("_", " ") || "Activity"
  return (
    details ||
    (targetId ? `${humanAction} on ${targetType} #${targetId}` : humanAction)
  )
}
