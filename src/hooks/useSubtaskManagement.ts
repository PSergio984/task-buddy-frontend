...
export interface UseSubtaskManagementReturn {
  ...
  setPendingSubtasks: React.Dispatch<
...
export function useSubtaskManagement(
  ...
  const handleToggleSubtask = useCallback(
    (subtaskId: number, completed: boolean) => {
  ...
  const handleReorderSubtasks = useCallback(
    (newSubtasks: (Subtask | { id: string; title: string })[]) => {
      if (isCreate) {
        setPendingSubtasks(
          newSubtasks as unknown as { id: string; title: string }[]
        )
      } else {
