import { type Tag, type Subtask, createIdempotencyKey } from "@/lib/api"
import {
  useAttachTag,
  useDetachTag,
  useCreateSubtask,
...
export function useTaskDrawerSync(): UseTaskDrawerSyncReturn {
  ...
  const syncTags = async (
    ...
    const existingTagsToAdd = currentTags.filter(
      (t) => t.id > 0 && !originalTagIds.includes(t.id)
    )
    const newTagsToCreate = currentTags.filter((t) => t.id < 0)

    await Promise.all([
      ...tagsToRemove.map((id) =>
        detachTag.mutateAsync({ taskId, tagId: id, silent })
      ),
      ...existingTagsToAdd.map((t) =>
        attachTag.mutateAsync({ taskId, tagId: t.id, silent })
      ),
    ])
  ...
  const syncSubtasks = async (
    ...
    const subtasksToUpdate = local.filter(
      (s) =>
        s.id > 0 &&
        (original.find((os) => os.id === s.id)?.title !== s.title ||
          original.find((os) => os.id === s.id)?.completed !== s.completed)
    )
    const subtasksToDelete = original.filter(
      (os) => !local.some((ls) => ls.id === os.id)
    )

    await Promise.all([
      ...subtasksToDelete.map((s) =>
        deleteSubtask.mutateAsync({ id: s.id, silent })
      ),
      ...subtasksToUpdate.map((s) =>
        updateSubtask.mutateAsync({
          id: s.id,
          updates: { title: s.title, completed: s.completed },
          silent,
        })
      ),
    ])
    ...
    const hasOrderingChanged =
      ...
      local.some((ls, i) => ls.id !== original[i]?.id)
