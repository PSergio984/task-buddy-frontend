...
export function SystemOverview({
  ...
  if (loading || !stats) {
    return (
      <Card className="overflow-hidden rounded-[2.5rem] border bg-background/50 p-8 shadow-2xl shadow-primary/5 backdrop-blur-xl">
  ...
  const displayPercentage = timeframeStats
    ? timeframeStats.percentage
    : task_stats.completion_percentage
  const displayCompleted = timeframeStats
    ? timeframeStats.completed
    : task_stats.completed_tasks
  const displayTotal = timeframeStats
    ? timeframeStats.total
    : task_stats.total_tasks
  const displayPending = timeframeStats
    ? timeframeStats.pending
    : task_stats.pending_tasks
  const displayDistribution = timeframeStats
    ? timeframeStats.distribution
    : tag_distribution
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-3">
            ...
            <motion.div
              ...
              className="h-3 w-full overflow-hidden rounded-full border border-border/50 bg-muted/30 p-0.5"
            >
              <motion.div
                className="h-full animate-shimmer rounded-full bg-gradient-to-r from-primary via-accent to-primary bg-[length:200%_100%]"
                initial={{ width: 0 }}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="group relative overflow-hidden rounded-2xl border bg-card/50 p-4 transition-all hover:bg-card">
              ...
              <p className="text-[10px] font-bold tracking-wider text-muted-foreground uppercase">
                Remaining
                {timeframeLabel && timeframeLabel !== "All Time"
                  ? ` · ${timeframeLabel}`
                  : ""}
              </p>
            ...
            <div className="group relative overflow-hidden rounded-2xl border border-accent/20 bg-accent/5 p-4 transition-all hover:bg-accent/10">
              ...
              <p className="text-[10px] font-bold tracking-wider text-accent/60 uppercase">
                Achieved
                {timeframeLabel && timeframeLabel !== "All Time"
                  ? ` · ${timeframeLabel}`
                  : ""}
            ...
            <div className="space-y-4 border-t border-border/50 pt-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                {displayDistribution
                  ...
                  .map(
                    (
                      tag: { tag_name: string; task_count: number },
                      idx: number
                    ) => (
                      <motion.div
                        key={tag.tag_name}
                      ...
                      >
                        ...
                        <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted/50">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{
                              width: `${displayTotal > 0 ? (tag.task_count / displayTotal) * 100 : 0}%`,
                            }}
                            transition={{
                              duration: 0.8,
                              delay: 0.5 + idx * 0.1,
